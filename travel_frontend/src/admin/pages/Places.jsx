...
import { fetchPlaces, createPlace, updatePlace, deletePlace } from '../../redux/slices/placeSlice';
...
const Places = () => {
  ...
  const handleEdit = (place) => {
    ...
    if (place.image) {
      setImagePreview(`http://localhost:8000/${place.image}`);
    }
    setShowModal(true);
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this place?')) {
      try {
        ...
        toast.error(error || 'Failed to delete place');
      }
    }
  };
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">Manage cities and places for your packages</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="text-center py-12">
          <div className="inline-block animate-spin rounded-full h-10 w-10 border-4 border-gray-300 border-t-purple-600"></div>
          <p className="text-gray-600 mt-3">Loading places...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {places?.map((place) => (
            <div key={place.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">
                    {place.packages_count || 0} packages
                  </span>
          ...
          <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="flex gap-3 justify-end">
                ...
                >
                  {editMode ? 'Update Place' : 'Create Place'}
                </button>
              </div>
            </form>
          </div>
